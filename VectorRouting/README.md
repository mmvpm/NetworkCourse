...
Сначала алгоритм сходится к верным расстояниям между вершинами (для графа из задания).  
Далее мы вручную изменяем стоимости некоторых ребер, после чего алгоритм постепенно подстраивает расстояния под изменённый граф.
